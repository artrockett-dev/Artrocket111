module SU_Furniture
	module Zip
		# ZipFile is modeled after java.util.zip.ZipFile from the Java SDK.
			...
			#   require 'zip/zip'
			#
			#   SU_Furniture::Zip::ZipFile.open("my.zip", SU_Furniture::Zip::ZipFile::CREATE) {
			#    |zipfile|
		...
		class ZipFile < ZipCentralDirectory
			...
			def initialize(fileName, create = nil, buffer = false)
				...
				@comment = ""
				case
					when ::File.exist?(fileName) && !buffer
          ...
          raise ZipError, "File #{fileName} not found"
			...
			class << self
				...
				def split(zip_file_name, segment_size=MAX_SEGMENT_SIZE, delete_zip_file=true, partial_zip_file_name=nil)
					raise ZipError, "File #{zip_file_name} not found" unless ::File.exist?(zip_file_name)
          ...
          "cannot open stream to directory entry - '#{newEntry}'"
			...
			def add(entry, srcPath, &continueOnExistsProc)
				continueOnExistsProc ||= proc { Zip.options[:continue_on_exist_proc] }
				check_entry_exist(entry, continueOnExistsProc, "add")
				newEntry = entry.kind_of?(ZipEntry) ? entry : ZipEntry.new(@name, entry.to_s)
			...
			def rename(entry, newName, &continueOnExistsProc)
				...
				check_entry_exist(newName, continueOnExistsProc, "rename")
			...
			def extract(entry, destPath, &onExistsProc)
				onExistsProc ||= proc { Zip.options[:on_exist_proc] }
				foundEntry = get_entry(entry)
			...
			def mkdir(entryName, permissionInt = 0755)
				if find_entry(entryName)
					raise Errno::EEXIST, "File exist - #{entryName}"
				end
          ...
          "'#{srcPath}' is not a directory"
			...
			def check_entry_exist(entryName, continueOnExistsProc, procedureName)
				continueOnExistsProc ||= proc { Zip.options[:continue_on_exist_proc] }
            ...
            procedureName + " failed. Entry #{entryName} already exist"
					end
				end
			end
