module SU_Furniture
	module Zip
      ...
      @filepath = nil
      if SU_Furniture::Zip::RUNNING_ON_WINDOWS
        @fstype = FSTYPE_FAT
				else
        @fstype = FSTYPE_UNIX
			end
      ...
      @dirty = false
			...
			def get_extra_attributes_from_path(path)	# :nodoc:
				unless SU_Furniture::Zip::RUNNING_ON_WINDOWS
					stat = file_stat(path)
					@unix_uid = stat.uid
					@unix_gid = stat.gid
          ...
          return NullInputStream.instance
			...
			def write_to_zip_output_stream(aZipOutputStream)  #:nodoc:all
				if @ftype == :directory
					...
					get_input_stream { |is| SU_Furniture::IOExtras.copy_stream(aZipOutputStream, is) }
					else
			...
			def write_file(destPath, continueOnExistsProc = proc { Zip.options[:continue_on_exist_proc] })
				if ::File.exist?(destPath) && ! yield(self, destPath)
					raise ZipDestinationFileExistsError,
          "Destination '#{destPath}' already exist"
				end
            ...
            "A file already exist with that name"
					end
				end
			...
			def create_symlink(destPath)
              ...
              "A symlink already exist with that name"
						end
						else
						raise ZipDestinationFileExistsError,
            "Cannot create symlink '#{destPath}'. "+
