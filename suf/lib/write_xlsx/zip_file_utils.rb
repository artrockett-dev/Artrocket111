...
#
require 'kconv'
# require File.dirname(__FILE__) + '/../zip/zipfilesystem'
require_relative 'zipfilesystem'
require 'fileutils'
...
module ZipFileUtils
  ...
  def self.zip(src, dest, options = {})
    ...
    FileUtils.rm_f(dest)
    SU_Furniture::Zip::ZipFile.open(dest, SU_Furniture::Zip::ZipFile::CREATE) do |zf|
      if File.file?(src)
  ...
  def self.unzip(src, dest, options = {})
    ...
    SU_Furniture::Zip::InputStream.open(src) do |is|
      loop do
