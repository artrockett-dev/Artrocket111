...
module SU_Furniture
  class SUInteractTool
    def initialize(panel_mode=0)
      ...
      @hover_message = $dc_strings.GetString(@hover_message)
      @is_animating = false
    ...
    def activate
      ...
      begin
        ...
        Sketchup::set_status_text $dc_strings.GetString(@hover_message)
          ...
          if layer.name.include?("опции") && layer.visible? && @layer_visible == []
            @model.layers.each { |l|
              if l.name.include?("открывание")
                l.remove_observer $SUFLayersObserver
                l.visible = true
    ...
    def draw(view)
		  draw_text(view, Geom::Point3d.new(30, 25, 0), "Удерживайте Shift для первоначального значения", @text_left_options)
      if @layer_visible != []
        draw_text(view, Geom::Point3d.new(view.vpwidth/2, 25, 0), "Включен слой: "+@layer_visible.join(", "), @text_default_options)
      end
    end
    def deactivate(view)
      @activate = false
      UI.start_timer(0.1, false) {
        if !@activate && !Change_Point.active
          begin
            @model.start_operation "layers", true, false, true
        ...
        ph.count.times { |pick_path_index|
          ...
          for i in 0..pick_list.length-1
            ...
            if entity.is_a?(Sketchup::ComponentInstance)
              ...
              @model.layers.each { |l| @options_layer = l.name if l.name.include?("Фасад_опции") && l.visible? || l.name.include?("Ручки_опции") && l.visible? || l.name.include?("Фасад_текстура") && l.visible? || l.name.include?("Каркас_текстура") && l.visible? || l.name.include?("Толщина_кромки") && l.visible? }
              ...
              if @onclick_action
                ...
                Sketchup::set_status_text $dc_strings.GetString("Click to activate.")
                ...
                if msg == SUF_STRINGS["Open"]
                  Sketchup::set_status_text("Процент открывания (от 10 до 175)", SB_VCB_LABEL)
                  Sketchup::set_status_text(@open_kf.to_s, SB_VCB_VALUE)
                end
                break
                else
                Sketchup::set_status_text $dc_strings.GetString("No click behaviors.")
    ...
    def click_hint(entity)
      if entity.definition.name.include?("edge_front")
        ...
        if @onclick_action.include?("ANIMATECUSTOM")
          ...
          msg = entity.definition.get_attribute("dynamic_attributes", "_"+@onclick_name+"_formlabel", $dc_strings.GetString("Click to activate."))
        end
      end
      return msg
    end#def
      ...
      for command in commands
        ...
        if function.index('animate') == 0 or function == 'set'
          ...
          if animate_type == "custom"
            ...
            length,error = @dc.parse_formula(length,onclick_entity)
            easein,error = @dc.parse_formula(easein,onclick_entity)
            easeout,error = @dc.parse_formula(easeout,onclick_entity)
            length = length.to_f
            ...
            if @shift_press == true
              ...
              if @onclick_entity.definition.name.include?("Texture")
                ...
                if x_max_x_formula == 'z_max_width+trim_z1+trim_z2'
                  entity.definition.set_attribute("dynamic_attributes", "_x_max_x_formula",'CHOOSE(napr_texture,z_max_width+trim_z1+trim_z2,z_max_length+trim_y1+trim_y2)')
                end
                if a0_lenz || a0_leny || a0_lenx
                  if a0_lenx*2.54 > x_max_x.to_f || a0_leny*2.54 > x_max_y.to_f || a0_lenz*2.54 > x_max_z.to_f
                    result = UI.messagebox(SUF_STRINGS["The width of the material is less than the width of the panel."]+"\n"+SUF_STRINGS["Change the direction of the texture and reduce the size?"], MB_YESNO)
    ...
    def find_entity_by_sheet_name(source_entity,sheet_name)
      ...
      if @dc.name_of_entity_is(source_entity,sheet_name)
        return [source_entity]
      end
      if source_entity.parent
        if !source_entity.parent.to_s.index('Sketchup::Model')
              ...
              if parent_entity.parent.is_a?(Sketchup::ComponentDefinition)
                for subentity in parent_entity.parent.entities
                  if subentity.is_a?(Sketchup::ComponentInstance) || subentity.is_a?(Sketchup::Group)
                    if @dc.name_of_entity_is(subentity,sheet_name,'parent')
                      return [subentity]
                    end
                  end
                end
        ...
        if source_entity.is_a?(Sketchup::ComponentInstance)
          for subentity in source_entity.definition.entities
            if subentity.is_a?(Sketchup::ComponentInstance) ||
              ...
              if @dc.name_of_entity_is(subentity,sheet_name,'children')
        ...
        for subentity in source_entity.parent.entities
          if subentity.is_a?(Sketchup::ComponentInstance) || subentity.is_a?(Sketchup::Group)
            if @dc.name_of_entity_is(subentity,sheet_name,'sibs')
              subentity_array.push(subentity)
            end
          end
        end
    ...
    def attach_animation(entity,attribute,targetval,length,easein,easeout)
      ...
      if @timer == nil
        @timer = UI.start_timer(1.0/12.0,true) {
          begin
            $SUFToolsObserver.register_timer_id(@timer)
            ...
            @animation_list.each do |animation|
              ...
              if fraction_complete >= 1.0
                ...
                if fraction_complete < easein_end
                  ...
                  fraction_eased = (1.0-easeout_start).zero? ? 1.0 : (fraction_complete-easeout_start)/
                  (1.0-easeout_start)
                  fraction_complete = fraction_complete + 
                ...
                if is_camera
                  ...
                  @model.active_view.camera.set neweye, 
                  newtarget, newup
            ...
            unless finished.empty?
              @animation_list.reject! { |a| finished.include?(a) || (!a['entity'].to_s.include?('Sketchup::Camera') && !a['entity'].valid?) }
            end
            @model.active_view.invalidate
    ...
    def redraw(entity,open)
      ...
      if x_formula && x_formula.include?('animation') && !entity.definition.get_attribute("dynamic_attributes", 'open') # двери купе
        local_origin_transform = entity.local_transformation
        entity.transform! local_origin_transform.inverse
        result_x,error = @dc.parse_formula(x_formula,entity,'x')
        new_pos = Geom::Point3d.new second_if_empty(result_x,entity.transformation.origin.x.to_f),result_y,result_z
        ...
        elsif open && open.to_i != 5 # фасады распашные
        if rotz_formula && rotz_formula.include?('a_rotz') # поворачивается сам фасад
          @dc.redraw(entity,false)
        ...
        if !open || open && open.to_i == 5 # ящики кухни и фасады
          local_origin_transform = entity.local_transformation
          entity.transform! local_origin_transform.inverse
          result_y,error = @dc.parse_formula(y_formula,entity,'y')
          new_pos = Geom::Point3d.new result_x,second_if_empty(result_y,entity.transformation.origin.y.to_f),result_z
          entity.transform! Geom::Transformation.new(entity.transformation.origin.vector_to(new_pos))
          entity.transform! local_origin_transform
        end
        
        elsif open && open.to_i == 5 && rotz_formula && rotz_formula.include?('a_rotz') # body фасада
        @dc.redraw(entity,false)
        
        elsif entity.definition.get_attribute("dynamic_attributes", 'su_type',"0")=='furniture' && entity.definition.get_attribute("dynamic_attributes", 'animation') # направляющие
        @dc.redraw(entity,false)
