module SU_Furniture
  class VisibleSide
    ...
    def draw(view)
      view.draw_text(Geom::Point3d.new(30, 25, 0), SUF_STRINGS["Specify the visible side"], @text_default_options)
      view.line_width=2
    ...
    def onLButtonDown(flags, x, y, view)
      if @exit
        ...
        @model.start_operation "Visible side", true
        ...
        if @essence.definition.get_attribute("dynamic_attributes", "visible_side")
          @essence.definition.delete_attribute("dynamic_attributes", "visible_side")
          @essence.definition.entities.grep(Sketchup::Face).each{|f| f.delete_attribute("dynamic_attributes", "visible_side")}
          else
          ...
          @ipface.set_attribute("dynamic_attributes", "visible_side",f_c.x)
          ...
          if @ipface.bounds.max.y < 2.4 || @ipface.bounds.max.z < 2.4
            heigt = [@ipface.bounds.max.y,@ipface.bounds.max.z].min-0.1
            else
            heigt = 4
          end
          group.entities.add_3d_text("v", TextAlignCenter, "Arial", false, false, heigt, 0.0, 0.0, false, 0.0)
					bounds = group.bounds
					proportion = bounds.width/bounds.height
          group.set_attribute('dynamic_attributes', "_lengthunits", "CENTIMETERS")
					set_att(group,"lenx",heigt,"LenX",'LenY*' + proportion.to_s)
					set_att(group,"leny",heigt,"LenY",heigt.to_s)
