module SU_Furniture
  class New_section
    def activate
      is_visible=$dlg_section.visible? if $dlg_section
      $dlg_section.close if $dlg_section && (is_visible==true)
      $dlg_section = UI::HtmlDialog.new({
        :dialog_title => "Make_section_"+PLUGIN_VERSION,
        :preferences_key => "section",
        :scrollable => true,
        :resizable => true,
        :width => 500,
        :height => 580,
      ...
      html_path = PATH + "/html/New_section.html"
      $dlg_section.set_file(html_path)	
      $dlg_section.show()
      $dlg_section.add_action_callback("get_data") { |web_dialog,action_name|
    ...
    def read_param
      param = []
      temp_hash = {}
      path_param = File.join(TEMP_PATH,"SUF","section.dat")
      ...
      content = File.readlines(File.join(PATH,"parameters","section.dat")).map(&:strip)
      content.each{|i|
        cont = i.strip.split("=")
      ...
      $dlg_section.execute_script(command)
    end#def
    ...
    def make_section(param)
      ...
      sel = @model.selection
      @model.start_operation('Make section', true)
      sel.grep(Sketchup::ComponentInstance).each{|ent|
        if ent.definition.get_attribute('dynamic_attributes', "description","0") == "Каркас"
          if @code_pos == "1"
      ...
      set_att(body,"description","body","Description")
      set_att(body,"su_type","Body")
      ...
      set_att(unit,"name","","Name",access,formlabel,formulaunits,units,'a03_name&" "&ABS(ROUND(a01_lenx*10,0))&_&ABS(ROUND(a01_leny*10,0))&_&ABS(ROUND(a01_lenz*10,0))')
      set_att(unit,"itemcode",@item_code,"ItemCode","NONE",formlabel,formulaunits,units,'a04_itemcode')
      set_att(unit,"description","Каркас","Description")
      ...
      set_att(unit,"hidden","0","Hidden")
      set_att(unit,"scaletool","120","ScaleTool",access,"ScaleTool",formulaunits,"STRING")
      
      set_att(unit,"a0","&#9660;",label,"VIEW","<font color=cc6600><b>Габариты секции<b></font>","STRING")
      set_att(unit,"a00_lenx",max_x,label,"TEXTBOX","<font color=cc0000>Ширина <font color=888888> 100-1810</font>","CENTIMETERS","MILLIMETERS")
      set_att(unit,"a00_leny",max_y,label,"TEXTBOX","<font color=009900>Глубина <font color=888888> 100-1810</font>","CENTIMETERS","MILLIMETERS")
      set_att(unit,"a00_lenz",max_z,label,"TEXTBOX","<font color=0033ff>Высота <font color=888888> 100-1810</font>","CENTIMETERS","MILLIMETERS")
      set_att(unit,"a01_lenx","",label,"VIEW","<font color=cc0000><b>Ширина<b></font><font color=888888> (реальная)</font>","CENTIMETERS","MILLIMETERS")
      set_att(unit,"a01_leny","",label,"VIEW","<font color=009900><b>Глубина<b></font><font color=888888> (реальная)</font>","CENTIMETERS","MILLIMETERS")
      set_att(unit,"a01_lenz","",label,"VIEW","<font color=0033ff><b>Высота<b></font><font color=888888> (реальная)</font>","CENTIMETERS","MILLIMETERS")
      set_att(unit,"a0_len","",label,"NONE",formlabel,"CENTIMETERS","MILLIMETERS",'SETLEN("a01_lenx",IF(a00_lenx<x_min_x,x_min_x,IF(a00_lenx>x_max_x,x_max_x,a00_lenx-trim_x1-trim_x2)))*SETLEN("a01_leny",IF(a00_leny<x_min_y,x_min_y,IF(a00_leny>x_max_y,x_max_y,a00_leny-trim_y1-trim_y2)))*SETLEN("a01_lenz",IF(a00_lenz<x_min_z,x_min_z,IF(a00_lenz>x_max_z,x_max_z,a00_lenz-trim_z1-trim_z2)));SETLABEL("a00_lenx",CONCATENATE(╡┴║,"cc0000",├Ширина┴╡┴║888888├┴,ROUND(x_min_x*10),│,ROUND(x_max_x*10),╞));SETLABEL("a00_leny",CONCATENATE(╡┴║,"009900",├Глубина┴╡┴║888888├┴,ROUND(x_min_y*10),│,ROUND(x_max_y*10),╞));SETLABEL("a00_lenz",CONCATENATE(╡┴║,"0033ff",├Высота┴╡┴║888888├┴,ROUND(x_min_z*10),│,ROUND(x_max_z*10),╞))')
      ...
      set_att(unit,"a02","&#9660;",label,"VIEW","<font color=cc6600><b>Параметры секции<b></font>","STRING")
      set_att(unit,"a03_name",@comp_name,label,"TEXTBOX","Название","STRING","STRING")
      set_att(unit,"a04_itemcode",@item_code,label,"TEXTBOX","Код изделия","STRING","STRING")
      set_att(unit,"b1_f_thickness",@frontal_thickness/25.4,label,"TEXTBOX","Толщина фасадов","CENTIMETERS","MILLIMETERS")
      set_att(unit,"b1_f_material","Фасад_Базовая",label,access,formlabel,"STRING","STRING")
      set_att(unit,"b1_p_thickness",@panel_thickness/25.4,label,"TEXTBOX","Толщина панелей","CENTIMETERS","MILLIMETERS")
      set_att(unit,"b1_p_material","ЛДСП_Базовая",label,access,formlabel,"STRING","STRING")
      set_att(unit,"b1_b_thickness",@back_thickness/25.4,label,"TEXTBOX","Толщина ЗС/дна","CENTIMETERS","MILLIMETERS")
      
      set_att(unit,"p1","&#9654;",label,"VIEW","<font color=cc6600><b>Оси привязки<b></font>","STRING")
      ...
      set_att(unit,"t1","&#9654;",label,"VIEW","<font color=cc6600><b>Отступы<b></font>","STRING")
      ...
      set_att(unit,"x_max_x","1000",label,access,formlabel,"STRING",units,'z_max_width+trim_x1+trim_x2')
      set_att(unit,"x_max_y","1000",label,access,formlabel,"STRING",units,'z_max_depth+trim_y1+trim_y2')
      set_att(unit,"x_max_z","1000",label,access,formlabel,"STRING",units,'z_max_height+trim_z1+trim_z2')
      set_att(unit,"x_min_x","1",label,access,formlabel,"STRING",units,'z_min_width')
      set_att(unit,"x_min_y","1",label,access,formlabel,"STRING",units,'z_min_depth')
      set_att(unit,"x_min_z","1",label,access,formlabel,"STRING",units,'z_min_height')
      
      set_att(unit,"z_max_width",@x_max_x/10,label,access,formlabel,"STRING")
      set_att(unit,"z_max_depth",@x_max_y/10,label,access,formlabel,"STRING")
      set_att(unit,"z_max_height",@x_max_z/10,label,access,formlabel,"STRING")
      set_att(unit,"z_min_width",@x_min_x/10,label,access,formlabel,"STRING")
      set_att(unit,"z_min_depth",@x_min_y/10,label,access,formlabel,"STRING")
      set_att(unit,"z_min_height",@x_min_z/10,label,access,formlabel,"STRING")
      ...
      set_att(unit,'y0_list',"1",label,access,formlabel,formulaunits,units,'SETLIST("y0_name_manual")')
      ...
      set_att(unit,"su_type","section",label,access,formlabel,formulaunits,units,formula,options)
    ...
    def explode_section
      ...
      ents = @model.active_entities
      sel = @model.selection
      @model.start_operation('Explode section', true)
      nested_components = []
      sel.grep(Sketchup::ComponentInstance).to_a.each { |entity|
        if entity.definition.get_attribute('dynamic_attributes', "su_type","0") == "section"
